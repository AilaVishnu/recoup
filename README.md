...
cp .env.example .env          # add your rzp_test_ keys + ANTHROPIC_API_KEY
python scripts/seed.py        # build the synthetic merchant
pytest -q                     # the bounds are the product; they get tested like it
```
...
| ✅ Policy engine — 13 bounds, 20 tests | `recoup/policy/rules.py` |
| ⬜ Action executors against Razorpay test mode | `recoup/execute/` |
| ⬜ Agent decision layer | `recoup/agent/` |
| ⬜ Eval harness — incremental lift, sensitivity sweep | `recoup/eval/` |
| ⬜ Dashboard + audit trail UI | `recoup/api/` |
